...
@genType
and entities = {
    @as("AnnouncementEvent") announcementEvent: entityStoreOperations<Entities.AnnouncementEvent.t>,
  }
      ...
      announcementEvent: {
        makeStoreOperatorEntity(
          ...
          ~getStore=db => db->InMemoryStore.getInMemTable(
            ~entityConfig=module(Entities.AnnouncementEvent)->Entities.entityModToInternal,
          )->(
            Utils.magic: InMemoryTable.Entity.t<Internal.entity> => InMemoryTable.Entity.t<
              Entities.AnnouncementEvent.t,
            >
          ),
          ~getKey=({id}) => id,
        )
