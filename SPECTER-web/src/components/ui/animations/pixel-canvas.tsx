...
class Pixel {
  ...
  constructor(
    ...
    this.width = cssWidth ?? canvas.width;
    this.height = cssHeight ?? canvas.height;
    this.ctx = context;
    this.x = x;
    this.y = y;
    this.color = color;
...
class PixelCanvasElement extends HTMLElement {
  ...
  getDistanceToCenter(x: number, y: number): number {
  ...
  createPixels() {
    ...
    for (let x = 0; x < this._cssWidth; x += this.gap) {
      for (let y = 0; y < this._cssHeight; y += this.gap) {
        const color =
...
declare global {
  interface HTMLElementTagNameMap {
    "pixel-canvas": PixelCanvasElement;
  }
}
...
const PixelCanvas = React.forwardRef<PixelCanvasElement, PixelCanvasProps>(
  (
    {
      gap,
      speed,
