...
export function RecoveryScriptBlock({
  ...
  return (
    <div className={cn("mt-6 rounded-xl border border-white/[0.08] bg-card/30 p-5 space-y-3", className)}>
      {collapsible ? (
        <button
          type="button"
        ...
        >
          {header}
          <ChevronDown
            className={`h-4 w-4 text-muted-foreground shrink-0 transition-transform ${sectionOpen ? "rotate-180" : ""}`}
          />
        </button>
      ) : (
        header
      ...
      {sectionOpen && (
        <>
          ...
          <div>
            ...
            >
              <span>{scriptOpen ? "Hide" : "Show"} recover.mjs</span>
          ...
          <div className="flex items-start gap-2 text-[11px] text-muted-foreground/80">
            ...
            <span>
              Each recovered key is re-checked against its address before it's printed — if the
              private key doesn't control the funds, the script says so.
            </span>
          </div>
        </>
      )}
